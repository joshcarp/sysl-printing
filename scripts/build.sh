...
if [[ ${CMD} = "build" ]]; then
    if [[ -n ${GOOS} && -n ${GOARCH} ]]; then
        GOOS=${GOOS} GOARCH=${GOARCH} go build -o ${OUT} -ldflags "${FLAGS}" -v github.com/joshcarp/sysl-printing/cmd/sysl
    else
        go build -o ${OUT} -ldflags "${FLAGS}" -v github.com/joshcarp/sysl-printing/cmd/sysl
    fi
elif [[ ${CMD} = "install" ]]; then
    go install -ldflags "${FLAGS}" -v ./cmd/sysl
fi
